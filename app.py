import os #透過os 套件進行檔案管理
import time #存檔 以時間為檔名 以利後續管理
import logging #方便開發者除錯的package
import subprocess #執行 command line 指令, 例如語音合成 edge-tts "檔名" --tone 10 --speed 10 
import tempfile #建立暫存檔案
import whisper #導入語音辨識
import requests #可以進行 get, post : 可以進行API 呼叫的套件
from datetime import datetime #時間格式 yyyy-mm-dd
from pathlib import Path #
#用來架設 API 服務的套件; 比較新的 fastapi進行API服務開發
from flask import Flask, request, abort, jsonify, send_file
#                                        轉成 json  傳遞檔案

from linebot import LineBotApi, WebhookHandler
from linebot.exceptions import InvalidSignatureError #可以顯示使用linebot時，錯誤訊息
from linebot.models import MessageEvent, AudioMessage, TextMessage, TextSendMessage, AudioSendMessage
#huggingface 提供的 transformer -> 載入模型, 分詞器等等
from transformers import AutoTokenizer, AutoModelForCausalLM

#pytorch;  大陸使用架構 paddlepaddle
import torch # 可以將模型載入GPU, 進行量化處理
import opencc # 繁體字 <-> 簡體字

app = Flask(__name__)#啟動flask服務
...
SERVER_PORT = 8080 #10000
line_bot_api = LineBotApi(LINE_CHANNEL_ACCESS_TOKEN)
line_handler = WebhookHandler(LINE_CHANNEL_SECRET)
# Whisper 和 LLM 模型設置
stt_model = whisper.load_model("base")
tokenizer = AutoTokenizer.from_pretrained("niwz/Mini-Chinese-Phi3")#500M
...
def answer_question(question):
    ...
    outputs = llm_model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], max_new_tokens=, pad_token_id=tokenizer.eos_token_id)
